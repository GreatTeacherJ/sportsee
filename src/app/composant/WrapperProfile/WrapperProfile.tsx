import styles from "@/app/composant/WrapperProfile/WrapperProfile.module.css";
import Image from "next/image";
import ProfileCard from "../ProfileCard/ProfileCard";
import { useContexteAPI, contextApi } from "@/contexts/context";
import { getStatUserActivity } from "@/utils/utilsAPI";

export default function WarppeProfil() {
	const { statistics, userActivity } = useContexteAPI(contextApi);

	if (!statistics) {
		return <p>loading ...</p>;
	}
	let totalDistances = 0;
	if (userActivity) {
		const { totalDistance } = getStatUserActivity(userActivity);

		totalDistances = totalDistance;
	}

	return (
		<section className={styles.wrapper}>
			<article className={styles.conversationBanner}>
				<div className={styles.textConainerAi}>
					<svg
						width="19"
						height="21"
						viewBox="0 0 19 21"
						fill="none"
						xmlns="http://www.w3.org/2000/svg"
					>
						<path
							d="M6.97444 5.20038C7.18569 4.51179 8.16047 4.51179 8.37171 5.20039L9.36071 8.4242C9.43181 8.65598 9.61324 8.83741 9.84502 8.90851L13.0688 9.89751C13.7574 10.1088 13.7574 11.0835 13.0688 11.2948L9.84502 12.2838C9.61324 12.3549 9.43181 12.5363 9.36071 12.7681L8.37171 15.9919C8.16046 16.6805 7.18569 16.6805 6.97444 15.9919L5.98545 12.7681C5.91434 12.5363 5.73292 12.3549 5.50114 12.2838L2.27732 11.2948C1.58873 11.0835 1.58873 10.1088 2.27732 9.89751L5.50114 8.90851C5.73292 8.83741 5.91434 8.65598 5.98545 8.4242L6.97444 5.20038Z"
							fill="#0B23F4"
						/>
						<path
							d="M12.8877 0.86755C12.9682 0.60523 13.3395 0.60523 13.42 0.867551L13.7968 2.09567C13.8238 2.18397 13.893 2.25308 13.9813 2.28017L15.2094 2.65693C15.4717 2.73741 15.4717 3.10875 15.2094 3.18922L13.9813 3.56598C13.893 3.59307 13.8238 3.66219 13.7968 3.75048L13.42 4.9786C13.3395 5.24092 12.9682 5.24092 12.8877 4.9786L12.5109 3.75048C12.4839 3.66219 12.4147 3.59307 12.3264 3.56598L11.0983 3.18922C10.836 3.10875 10.836 2.73741 11.0983 2.65693L12.3264 2.28017C12.4147 2.25308 12.4839 2.18397 12.5109 2.09567L12.8877 0.86755Z"
							fill="#0B23F4"
						/>
						<path
							d="M14.6148 13.6159C14.7255 13.2553 15.2361 13.2553 15.3467 13.6159L15.8648 15.3046C15.902 15.426 15.9971 15.5211 16.1185 15.5583L17.8071 16.0763C18.1678 16.187 18.1678 16.6976 17.8071 16.8082L16.1185 17.3263C15.9971 17.3635 15.902 17.4586 15.8648 17.58L15.3467 19.2686C15.2361 19.6293 14.7255 19.6293 14.6148 19.2686L14.0968 17.58C14.0595 17.4586 13.9645 17.3635 13.8431 17.3263L12.1544 16.8082C11.7937 16.6976 11.7937 16.187 12.1544 16.0763L13.8431 15.5583C13.9645 15.5211 14.0595 15.426 14.0968 15.3046L14.6148 13.6159Z"
							fill="#0B23F4"
						/>
					</svg>

					<p className={styles.conversationText}>
						Posez vos questions sur votre programme, vos performances ou vos
						objectifs.
					</p>
				</div>
				<a href="/coach-ai" className={styles.conversationButton}>
					Lancer une conversation
				</a>
			</article>

			{/* User profile card */}
			<article className={styles.profileCard}>
				<ProfileCard />

				<p className={styles.distanceLabel}>Distance totale parcourue</p>

				<div className={styles.distanceBadge}>
					<Image
						src="/images/OUTLINE.png"
						alt="icone"
						width={34}
						height={34}
					/>
					<span className={styles.distanceValue}>
						{totalDistances.toFixed(2)} km
					</span>
				</div>
			</article>
		</section>
	);
}
